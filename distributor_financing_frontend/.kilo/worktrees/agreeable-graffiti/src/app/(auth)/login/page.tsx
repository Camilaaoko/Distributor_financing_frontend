...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let resolvedPermissions = response.permissions;
      ...
      if (response.token && response.token.includes('.')) {
        try {
          ...
          bankId = payload.bankId || payload.parentEntityId || payload.tenantId || payload.entityId;
          bankName = payload.bankName || payload.tenantName;
          if (!resolvedRole) resolvedRole = payload.role || payload.roleCode || payload.userRole;
          if (!resolvedPermissions && (payload.permissions || payload.authorities)) {
            resolvedPermissions = payload.permissions || payload.authorities;
          }
          if (!userEmail) userEmail = payload.email || (payload.sub && payload.sub.includes('@') ? payload.sub : undefined);
          if (!userUsername) {
            if (payload.firstName && payload.lastName) {
              userUsername = `${payload.firstName} ${payload.lastName}`;
            } else if (payload.name) {
              userUsername = payload.name;
              ...
              userUsername = payload.sub;
            }
          }
        } catch {}
      }

      if (!userUsername && (response as any).firstName && (response as any).lastName) {
        userUsername = `${(response as any).firstName} ${(response as any).lastName}`;
      }
      ...
      // 3. If bankId or full name not in token, resolve from /api/onboarding/banks/admins or /api/onboarding/bank-users for bank roles
      if (resolvedRole && resolvedRole.toUpperCase().includes('BANK')) {
        try {
          const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://172.16.10.64:8080';
          const [adminsRes, bankUsersRes] = await Promise.allSettled([
            fetch(`${baseUrl}/api/onboarding/banks/admins`, {
              headers: { Authorization: `Bearer ${response.token}` }
            }),
            fetch(`${baseUrl}/api/onboarding/bank-users`, {
              headers: { Authorization: `Bearer ${response.token}` }
            })
          ]);

          if (adminsRes.status === 'fulfilled' && adminsRes.value.ok) {
            const admins = await adminsRes.value.json();
            if (Array.isArray(admins)) {
              const matched = admins.find(
                (a: any) => a.email && a.email.toLowerCase().trim() === (userEmail || '').toLowerCase().trim()
              );
              if (matched) {
                if (!bankId) bankId = matched.bankId;
                if (!bankName) bankName = matched.bankName;
                if (matched.firstName && matched.lastName) {
                  userUsername = `${matched.firstName} ${matched.lastName}`;
                }
              }
            }
          }

          if (bankUsersRes.status === 'fulfilled' && bankUsersRes.value.ok) {
            const bankUsers = await bankUsersRes.value.json();
            if (Array.isArray(bankUsers)) {
              const matched = bankUsers.find(
                (u: any) => u.email && u.email.toLowerCase().trim() === (userEmail || '').toLowerCase().trim()
              );
              if (matched) {
                if (!bankId) bankId = matched.bankId || matched.entityId;
                if (!bankName) bankName = matched.bankName;
                if (matched.firstName && matched.lastName) {
                  userUsername = `${matched.firstName} ${matched.lastName}`;
                }
