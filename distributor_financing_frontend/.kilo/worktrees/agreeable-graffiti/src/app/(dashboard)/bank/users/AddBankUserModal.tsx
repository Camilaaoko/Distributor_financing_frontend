...
import { Upload, ShieldCheck, Plus, ExternalLink } from 'lucide-react';
...
import type { RoleResponse } from '@/types/onboarding';
...
export function AddBankUserModal({ open, onClose, onCreate, isSubmitting }: AddBankUserModalProps) {
      ...
      roleManagementApi
        ...
        .then((roles) => {
          ...
          if (roles && roles.length > 0) {
            ...
            const defaultId = unassigned?.id || roles[0]?.id || '';
            setForm((prev) => ({
              ...prev,
              roleId: prev.roleId || defaultId,
            }));
          }
        })
        .catch(() => {})
  ...
  const handleSubmit = async () => {
    ...
    if (!firstName || !lastName || !email || !phone || !nationalId || !employeeNumber || !roleId) {
      setError('Please fill in all required fields, including the role assignment.');
      return;
    }
