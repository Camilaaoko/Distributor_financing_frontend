...
const COLUMNS: { key: SortKey | 'emailPhone' | 'select' | 'actions'; label: string; sortable: boolean }[] = [
              ...
              <tr>
                <td colSpan={COLUMNS.length} className="px-4 py-16 text-center">
                  <div className="flex flex-col items-center gap-2 text-slate-400">
                    <Inbox size={32} />
                    <p className="text-sm font-semibold text-slate-500">No bank users found</p>
            ...
            {!isLoading && rows.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${roleBadgeColor(row.role)}`}>
                    {row.role === 'BANK_MAKER' ? 'Maker' : 'Checker'}
                  </span>
                </td>
...
function roleBadgeColor(role: string) {
  return role === 'BANK_MAKER' ? 'bg-blue-100 text-[#1F4DA8]' : 'bg-blue-50 text-[#3A6FD8]';
}