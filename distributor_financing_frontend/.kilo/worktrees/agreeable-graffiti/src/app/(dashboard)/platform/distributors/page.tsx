...
export default function PlatformDistributorsPage() {
    ...
    try {
      ...
      (userList || []).forEach((u) => {
        ...
        combined.push({
          ...
          distributorName: u.distributorName || (u.location ? `${u.location} Distributor` : 'Distributor Partner'),
          businessPermitNumber: u.businessPermitNumber,
          location: u.location,
          accountNumber: u.accountNumber,
          firstName: u.firstName || 'Dealer',
      ...
      // 2. Ensure all distributor entities are present
      (distList || []).forEach((d) => {
        ...
        if (!hasMatchingUser) {
          combined.push({
            id: d.id || `dist-${Math.random()}`,
            isUser: false,
            distributorId: d.id,
            distributorName: d.businessName || 'Distributor Partner',
            businessPermitNumber: d.businessPermitNumber,
            location: d.location,
  ...
  const handleToggleStatus = async (item: DisplayDistributorRow) => {
    ...
    try {
      if (isCurrentlyActive) {
        ...
        await distributorApi.deactivateDistributorAdmin(item.id);
        toast.success(`Distributor "${item.distributorName}" deactivated.`);
      } else {
        // Reactivate
        await distributorApi.activateDistributorAdmin(item.id);
        toast.success(`Distributor "${item.distributorName}" activated.`);
      }
      await loadData();
    } catch (err) {
      toast.error(getErrorMessage(err, `Failed to ${isCurrentlyActive ? 'deactivate' : 'activate'} distributor.`));
    } finally {
      setTogglingId(null);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Oversee distributor/dealer networks and manage primary administrator &amp; staff accounts across the ecosystem.
          </p>
        </div>
