...
import platformStyles from '@/components/platform/PlatformDashboard.module.css';
import styles from './BankUsers.module.css';
...
export default function PlatformBankUsersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const loadedBanks = banksData.items;
      ...
      try {
        ...
        if (!loadedBanks.length) throw new Error('Unable to load bank users. Please retry.');
        const userPromises = loadedBanks.map((b) =>
          bankOnboardingApi.getBankUsersByBankId(b.id)
        );
  ...
  const handleToggleStatus = async (user: EnhancedBankUser) => {
    ...
    try {
      if (isCurrentlyActive) {
        ...
        toast.success(`${user.firstName || 'User'} ${user.lastName || ''} has been deactivated.`);
      } else {
        await bankOnboardingApi.activateBankUser(user.id);
        toast.success(`${user.firstName || 'User'} ${user.lastName || ''} has been activated.`);
      }

      setUsers((prev) =>
        prev.map((u) =>
          u.id === user.id ? { ...u, status: newStatus, active: !isCurrentlyActive } : u
        )
      );
    } catch (err) {
  ...
  return (
    <div className={`${platformStyles.dashboard} ${styles.page}`}>
      <nav aria-label="Breadcrumb" className={styles.breadcrumb}><Link href="/platform">Platform</Link><span>/</span><span>Bank Users</span></nav>
      <header className={styles.heading}>
        <div><h1>Bank Users</h1><p>Manage bank staff, roles, and account access across partner branches.</p></div>
        <button onClick={loadData} disabled={isLoading} className={styles.refresh}><RefreshCw size={15} className={isLoading ? 'animate-spin' : ''} />{isLoading ? 'Refreshing…' : 'Refresh'}</button>
      </header>
      {error && <div role="alert" className={platformStyles.error}><span className={platformStyles.errorIcon}><AlertCircle size={24} /></span><span className={platformStyles.errorMessage}>{error}</span><button onClick={loadData} disabled={isLoading}>{isLoading ? 'Retrying…' : 'Retry'}</button></div>}
      <section aria-label="Bank user statistics" className={platformStyles.metrics}>
        {[
          { label: 'Total Bank Users', value: totalUsersCount, icon: Users, tone: 'blue', note: 'Registered bank staff' },
          { label: 'Active Users', value: activeUsersCount, icon: CheckCircle2, tone: 'green', note: 'Currently active accounts' },
          { label: 'Suspended / Inactive', value: suspendedUsersCount, icon: Clock, tone: 'amber', note: 'Restricted accounts' },
          { label: 'Partner Banks', value: totalBanksRepresented, icon: Landmark, tone: 'violet', note: 'Banks represented in this list' },
        ].map(metric => <article key={metric.label} className={platformStyles.metric} aria-busy={isLoading}><span className={`${platformStyles.metricIcon} ${platformStyles[metric.tone]}`}><metric.icon size={24} /></span><div><h2>{metric.label}</h2><strong>{isLoading || error ? '—' : metric.value.toLocaleString()}</strong><p>{metric.note}</p>{error && <span className={platformStyles.metricUnavailable}>Unavailable</span>}</div></article>)}
      </section>
      {/* Filter and Classification Bar */}
      <section aria-label="Filter bank users" className={styles.filters}>
        <div className="flex flex-col lg:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl shrink-0 w-full sm:w-auto justify-center">
            <button
              type="button"
              onClick={() => setViewMode('table')} aria-pressed={viewMode === 'table'}
            ...
            <button
              type="button"
              onClick={() => setViewMode('grouped')} aria-pressed={viewMode === 'grouped'}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
      ...
      <div className={styles.resultsHeading}><div><h2>Bank Users Directory</h2><p aria-live="polite">{isLoading ? 'Loading users…' : error ? 'User data unavailable' : `${filteredUsers.length} of ${users.length} users`}</p></div><button onClick={() => { setSearch(''); setSelectedBankId('ALL'); setSelectedBranch('ALL'); setRoleFilter('ALL'); setStatusFilter('ALL'); }} disabled={!search && selectedBankId === 'ALL' && selectedBranch === 'ALL' && roleFilter === 'ALL' && statusFilter === 'ALL'}>Clear filters</button></div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-16 text-center shadow-xs space-y-3">
          ...
          <h3 className="text-base font-bold text-slate-800">{error ? 'Unable to load bank users' : 'No bank users found'}</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {error ? 'Use Retry above to reload the directory.' : search || selectedBankId !== 'ALL' || selectedBranch !== 'ALL' || roleFilter !== 'ALL' || statusFilter !== 'ALL'
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-slate-400 font-mono">
                              ID: {user.nationalIdNumber || user.employeeId || user.id}
                            </p>
                          </div>
                        </div>
                      </td>
