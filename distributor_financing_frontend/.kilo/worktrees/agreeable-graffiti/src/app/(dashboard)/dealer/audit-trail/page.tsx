...
export default function DistributorAuditTrailPage() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load distributor audit trail.'));
    } finally {
      setIsLoading(false);
    }
  }, []);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Real-time audit log of your distributor profile changes, financing requests, invoice submissions, and user logins.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-xs space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {search || categoryFilter !== 'ALL' || dateRange.preset !== 'ALL' || dateRange.startDate
              ...
              : 'Audit events will appear here as orders, invoices, and financing requests are processed.'}
          </p>
        </div>
      ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-emerald-50 text-emerald-700 flex items-center justify-center font-bold text-xs">
                          {log.userFullName?.charAt(0) || log.userId?.charAt(0) || 'D'}
                        </div>
                        <div>
