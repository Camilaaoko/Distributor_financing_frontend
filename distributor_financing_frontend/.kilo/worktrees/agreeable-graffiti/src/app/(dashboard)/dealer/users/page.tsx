...
import { distributorApi, roleManagementApi } from '@/services/onboarding-api.service';
...
export default function DistributorUsersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [usersData, rolesData] = await Promise.all([
        distributorApi.getDistributorUsers(),
      ...
      setError(getErrorMessage(err, 'Failed to load distributor staff users.'));
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleCreateUser = async () => {
    ...
    if (!firstName || !lastName || !email || !phone || !nationalId || !employeeId || !roleId) {
      setModalError('Please fill in all required fields, including role selection.');
      return;
    }

    if (!PHONE_REGEX.test(phone)) {
      setModalError('Invalid phone format. Please use 07XXXXXXXX, 01XXXXXXXX, or +254XXXXXXXXX.');
      return;
    }

    if (!NATIONAL_ID_REGEX.test(nationalId)) {
      setModalError('National ID must be an 8-digit number (e.g. 12345678).');
      return;
    }
    ...
    try {
      await distributorApi.onboardDistributorUser({
        firstName,
        lastName,
        email,
        phoneNumber: phone,
      ...
      toast.success(`Distributor user account created for ${firstName} ${lastName}.`);
      setAddModalOpen(false);
      await loadData();
    } catch (err) {
      setModalError(getErrorMessage(err, 'Failed to create distributor user.'));
    } finally {
  ...
  const handleSaveEdit = async () => {
    ...
    setIsSubmitting(true);
    try {
      await distributorApi.updateDistributorUser(editTarget.id, {
  ...
  const handleDeleteUser = async (user: UserResponse) => {
    if (!user.id) return;
    if (!confirm(`Are you sure you want to deactivate ${user.firstName || 'user'} ${user.lastName || ''}?`)) return;
    setDeletingId(user.id);
    try {
      await distributorApi.deactivateDistributorUser(user.id);
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Manage order makers, loan drawdown authorizers, and settlement payment officers.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-600">Loading distributor users...</p>
        </div>
      ) : error ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-sm space-y-3">
          ...
          <h3 className="text-base font-bold text-slate-800">No Distributor Users Found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {search
              ? 'No users match your search criteria.'
              : 'Add your first ordering maker or drawdown checker account.'}
          </p>
          {!search && (
            <button
              onClick={handleOpenAdd}
      ...
      <Modal
        ...
        title="Add Distributor Staff User"
        description="Create an internal user account and assign operational roles."
        size="lg"
        footer={
          <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormField label="Corporate Email" required hint="Credentials will be dispatched here">
              <TextInput
                ...
                placeholder="s.otieno@distributor.co.ke"
              />
            </FormField>
            ...
            <FormField label="Employee Number / ID" required hint="Company staff code">
              <TextInput
                value={form.employeeId}
                onChange={(e) => setForm({ ...form, employeeId: e.target.value })}
                placeholder="DST-OPS-012"
              />
            </FormField>
