...
const DIST_FALLBACK_PERMISSIONS: PermissionDto[] = [
  { code: 'DRAWDOWN_REQUEST', description: 'Prepare and submit credit drawdown requests against purchase orders (Maker)' },
  { code: 'DRAWDOWN_APPROVE', description: 'Authorize and legally sign off loan terms and drawdowns (Checker)' },
  { code: 'REPAYMENT_INITIATE', description: 'Trigger loan repayments via M-Pesa, EFT, or bank debit' },
  { code: 'VIEW_STATEMENTS', description: 'View financial ledger statements, invoices, and repayment schedules' },
  { code: 'VIEW_PROFILE', description: 'Inspect approved credit limits, utilized balances, and terms' },
  { code: 'USER_MANAGEMENT', description: 'Manage internal distributor staff accounts' },
  { code: 'ROLE_MANAGEMENT', description: 'Create and configure custom distributor roles' },
];
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [rolesData, permsData] = await Promise.all([
        ...
        roleManagementApi.getAvailablePermissions().catch(() => DIST_FALLBACK_PERMISSIONS),
      ]);
      setRoles(rolesData || []);
      setAvailablePermissions(permsData && permsData.length > 0 ? permsData : DIST_FALLBACK_PERMISSIONS);
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to load distributor roles and permissions.'));
    } finally {
      setIsLoading(false);
    }
  }, []);
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
            ...
            Distributor Roles &amp; Permissions
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Configure custom operational roles and permissions for distributor order makers, authorizers, and payment officers.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200/80 shadow-sm">
        <div className="relative w-full sm:w-80">
          ...
          <TextInput
            ...
            placeholder="Search distributor roles..."
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-600">Loading distributor roles...</p>
        </div>
      ) : error ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-sm space-y-4">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-800">No Distributor Roles Found</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Create custom roles for your ordering, accounting, and drawdown sign-off personnel.
            </p>
          </div>
          {!search && (
            <button
                  ...
                  <div className="flex items-center gap-2 shrink-0 border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-100">
                    ...
                    {!role.isAssigned && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-xl transition-colors disabled:opacity-50"
                      >
                        {deletingId === role.id ? (
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100 bg-slate-50/50 p-6 space-y-4">
                    ...
                    {(role.permissions || []).length === 0 ? (
                      <p className="text-xs text-slate-400 italic">No granular permissions attached.</p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                        {(role.permissions || []).map((pCode, idx) => (
                          <div
                            key={`${pCode}-${idx}`}
                            className="bg-white p-2.5 rounded-xl border border-slate-200/60 shadow-2xs flex items-center gap-2 text-xs"
                          >
                            <ShieldCheck className="h-3.5 w-3.5 text-[#1F4DA8] shrink-0" />
      ...
      <Modal
        ...
        title="Create Distributor Role"
        description="Define a custom role for your staff and assign operational permissions."
        size="xl"
        footer={
          <>
      ...
      >
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <FormField label="Role Name" required hint="e.g. Drawdown Maker, Payment Officer">
              <TextInput
                value={roleName}
                onChange={(e) => setRoleName(e.target.value)}
                placeholder="Drawdown Maker"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Select permissions authorized for this distributor role.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[320px] overflow-y-auto pr-1">
              {availablePermissions.map((perm) => {
                const code = (perm.code || perm.name || '') as PermissionCode;
