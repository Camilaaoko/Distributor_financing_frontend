...
export function CreditLimitMeter({ profile, isLoading, onRefreshed }: CreditLimitMeterProps) {
  const toast = useToast();
  const [isRefreshingScore, setIsRefreshingScore] = useState(false);

  const creditLimit = profile?.creditLimit ?? 0;
  const utilizedAmount = profile?.utilizedAmount ?? 0;
  const availableCredit = profile?.availableCredit ?? (creditLimit > 0 ? Math.max(0, creditLimit - utilizedAmount) : 0);
  ...
  // Credit risk tier
  let riskTier = 'Assessment Pending';
  let tierBadgeBg = 'bg-slate-100 text-slate-600 border-slate-200';
  let maxAdvanceRate = '80%';

  if (hasScore) {
    if (creditScore >= 800) {
      riskTier = `Platinum Tier (${creditScore})`;
      tierBadgeBg = 'bg-indigo-50 text-indigo-700 border-indigo-200';
      maxAdvanceRate = '90%';
    } else if (creditScore >= 650) {
      riskTier = `Gold Tier (${creditScore})`;
      tierBadgeBg = 'bg-amber-50 text-amber-700 border-amber-200';
      maxAdvanceRate = '80%';
    } else {
      riskTier = `Silver Tier (${creditScore})`;
      tierBadgeBg = 'bg-slate-100 text-slate-700 border-slate-300';
      maxAdvanceRate = '70%';
    }
  }
  ...
  return (
    <div className="bg-gradient-to-br from-white via-slate-50/50 to-blue-50/30 rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-xs space-y-5">
      {/* Top Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${tierBadgeBg}`}>
