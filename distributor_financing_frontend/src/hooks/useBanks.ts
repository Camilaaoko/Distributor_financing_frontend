...
export function useBanks() {
  ...
  const createBank = useCallback(
    async (input: { bankCode: string; branchCode?: string; name?: string; bankName?: string; location?: string }): Promise<boolean> => {
      setIsMutating(true);
      try {
        await platformService.createBank({
          bankCode: input.bankCode,
          branchCode: input.branchCode || '',
          name: input.name || input.bankName,
          bankName: input.bankName || input.name,
          location: input.location,
        });
        toast.success(`Partner Bank ${input.name || input.bankName || input.bankCode} onboarded successfully.`);
        await refresh();
        return true;
      } catch (err) {
