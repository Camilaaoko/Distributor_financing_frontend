...
export default function PlatformRolesPage() {
  ...
  const handleToggleStatus = async (role: RoleResponse) => {
    const isCurrentlyActive = role.status === 'ACTIVE' || role.status === 'Active';
    ...
    try {
      if (isCurrentlyActive) {
        ...
        toast.success(`Role "${role.name}" activated.`);
      }
      await loadData();
    } catch (err) {
  ...
  const activeRolesCount = roles.filter((r) => r.status === 'ACTIVE' || r.status === 'Active').length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          filteredRoles.map((r) => {
            ...
            const isActive = r.status === 'ACTIVE' || r.status === 'Active' || r.status === undefined;
                ...
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <h3 className="font-bold text-slate-900 text-base mt-1.5 leading-snug">{r.name}</h3>
                    </div>
