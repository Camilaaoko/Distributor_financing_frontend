...
export default function RepaymentsPage() {
  return <RepaymentsDashboard mode="dealer" />;
              ...
              {isLoadingFacilities ? (
                ...
                facilities.map((f) => (
                  <option key={f.id} value={f.id}>
                    {f.id} - {f.distributorName} (Outstanding: {f.outstanding.toLocaleString()} {f.currency})
                  </option>
                ))
              )}
            </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-white border-slate-200/80 p-5 rounded-xl shadow-sm">
            ...
            <div className="text-2xl font-bold text-slate-900">
              {facilities.length > 0 
                ? facilities.reduce((sum, f) => sum + f.outstanding, 0).toLocaleString() 
                : '0'} KES
            </div>
              ...
              {isLoadingRepayments ? (
                ...
                repayments.map((rep) => (
                  <tr key={rep.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-3 font-bold text-emerald-600">{rep.totalAmount.toLocaleString()}</td>
                    <td className="p-3 text-slate-500">{rep.principalAllocated.toLocaleString()} / {rep.penaltiesAllocated.toLocaleString()}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 max-w-md w-full p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-200/60">
                ...
                <div className="text-slate-500">Amount: {selectedRepayment.totalAmount.toLocaleString()}</div>
              </div>
