  ...
  const loadReferrals = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const mapped: Referral[] = data.map((d) => ({
          id: d.id,
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 max-w-md w-full p-6 space-y-4">
            ...
            <div className="text-center space-y-4">
              ...
              <button
                onClick={() => { setShowReferralModal(false); router.push("/manufacturer/refer-distributor"); }}
                className="w-full px-4 py-2.5 bg-[#F97316] hover:bg-orange-600 text-white text-sm font-semibold rounded-lg transition-colors cursor-pointer"
              >
                Start Referral
              </button>
              <button
