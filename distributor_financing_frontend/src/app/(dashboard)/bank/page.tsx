...
export default function BankDashboardPage() {
  ...
  const welcomeName = user
    ? `${(user as any).firstName || ''} ${(user as any).lastName || ''}`.trim() || user.username || user.email || 'Bank Admin'
    : 'Bank Admin';
  const tenantName = (user as any)?.bankName || (user as any)?.tenantName || 'EMTech Financing Portal';
  const avatarLetter = (((user as any)?.firstName?.[0]) || user?.username?.[0] || welcomeName[0] || 'B').toUpperCase();
      ...
      {stats.map((stat) => {
        ...
        return (
          <StatCard
            ...
            href={(stat as any).href || iconCfg.defaultHref}
          />
        );
      })}
    </div>
