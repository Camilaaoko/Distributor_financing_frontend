...
export function ApproveDistributorModal({ approval, open, onClose, onApproved }: ApproveDistributorModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { bankId: resolvedBankId } = await resolveCurrentBank();
      const effectiveBankId = (approval as any).bankId || resolvedBankId || 'bank_01';
      const effectiveMfgId = (approval as any).manufacturerId || 'mfg_01';

      try {
        await LoansServiceClient.setupDistributor({
          distributorId: distEntityId,
          bankId: effectiveBankId,
          manufacturerId: effectiveMfgId,
      ...
      // 4. Adjust / Provision revolving loan profile in Loans Microservice (Idempotent)
      try {
        let profileId: number | string | undefined;

        // Check if profile already exists for this distributor
        const existingProfileRes = await distributorLoanProfilesApi.getProfileByDistributorId(distEntityId).catch(() => null);
        const existingProfile = (existingProfileRes as any)?.result || existingProfileRes;

        if (existingProfile && (existingProfile.id || existingProfile.distributorId)) {
          profileId = existingProfile.id;
          console.log('[DISTRIBUTOR_IDENTITY] Existing loan profile found for distributor:', distEntityId, 'profileId:', profileId);

          // Adjust credit limit on existing profile
          await distributorLoanProfilesApi.adjustCreditLimit(distEntityId, {
            newCreditLimit: Number(creditLimit),
            reason: isCustomOverride && overrideReason ? overrideReason : 'Sanctioned credit limit update',
          }).catch((adjErr) => console.warn('Credit limit adjustment notice:', adjErr));
        } else {
          // Create new loan profile if not existing
          try {
            const profileRes = await distributorLoanProfilesApi.createLoanProfile({
              ...
              gracePeriodDays: 3,
              financingModelId: Number(financingModelId) || 1,
              distributorEmail: em || approval.email || '',
              distributorContactName: `${fn} ${ln}`.trim() || approval.companyName || '',
            });
            profileId = (profileRes as any)?.result?.id || (profileRes as any)?.id;
          } catch (createErr: any) {
            const createMsg = String(createErr?.response?.data?.message || createErr?.message || '');
            if (createMsg.includes('already exists') || createErr?.response?.status === 409) {
              // Fetch existing and adjust
              const fallbackProfile = await distributorLoanProfilesApi.getProfileByDistributorId(distEntityId).catch(() => null);
              const fb = (fallbackProfile as any)?.result || fallbackProfile;
              profileId = fb?.id;
              await distributorLoanProfilesApi.adjustCreditLimit(distEntityId, {
                newCreditLimit: Number(creditLimit),
                reason: isCustomOverride && overrideReason ? overrideReason : 'Sanctioned credit limit update',
        ...
        if (profileId) {
          ...
          }).catch((apprErr) => console.warn('Profile approval notice (may already be active):', apprErr));
        }
      } catch (profileErr) {
