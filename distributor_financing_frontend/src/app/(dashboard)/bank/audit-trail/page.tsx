...
export default function BankAuditTrailPage() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load bank audit logs.'));
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const tenantTypes = Array.from(new Set(logs.map((l) => l.tenantType).filter(Boolean)));
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const matchesTenant = tenantTypeFilter === 'ALL' || log.tenantType === tenantTypeFilter;
    ...
    return matchesSearch && matchesCategory && matchesTenant && matchesDate;
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Immutable log of all user activities, loan reviews, disbursement authorizations, and role modifications.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto justify-end">
          ...
          {tenantTypes.length > 0 && (
            <Select
              ...
              className="text-xs font-semibold w-auto min-w-[130px]"
            >
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center shadow-xs space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {search || categoryFilter !== 'ALL' || dateRange.preset !== 'ALL' || dateRange.startDate
              ? 'No audit log entries match your current filter and date criteria.'
              : 'Audit events will automatically appear here as bank users perform operations.'}
          </p>
        </div>
      ) : (
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-blue-50 text-[#1F4DA8] flex items-center justify-center font-bold text-xs">
                          {log.userFullName?.charAt(0) || log.userId?.charAt(0) || 'U'}
                        </div>
                        <div>
