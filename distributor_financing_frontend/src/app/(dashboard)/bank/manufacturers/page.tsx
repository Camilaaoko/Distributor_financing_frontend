...
export default function BankManufacturersPage() {
  ...
  const handleToggleStatus = async (m: ManufacturerResponse) => {
    const isActive = m.status === 'ACTIVE';
    setTogglingId(m.id);
    try {
      if (isActive) {
        await manufacturerApi.deactivateManufacturerAdmin(m.id).catch(() =>
          manufacturerApi.setManufacturerAdminStatus(m.id, false)
        );
        toast.success(`Manufacturer "${m.name}" deactivated.`);
      } else {
        await manufacturerApi.activateManufacturerAdmin(m.id).catch(() =>
          manufacturerApi.setManufacturerAdminStatus(m.id, true)
        );
        toast.success(`Manufacturer "${m.name}" activated.`);
      }
      loadData();
    } catch (err) {
      toast.error(getErrorMessage(err, 'Failed to update manufacturer status.'));
    } finally {
      setTogglingId(null);
    }
  };
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-50 text-[#1F4DA8] border border-blue-200 shadow-xs">
              Branch Manufacturer Management
            </span>
          </div>
          <p className="text-sm text-slate-500 mt-1">
            Onboard and manage corporate manufacturers whose distributor supply chains are financed by your bank branches.
          </p>
        </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-14 text-center text-slate-400">
                    ...
                    <p className="text-xs text-slate-400 mt-1 mb-3">Onboard your first anchor corporate manufacturer to begin financing their distributor network.</p>
                    <button
                      onClick={() => setAddModalOpen(true)}
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#1F4DA8] hover:bg-[#3A6FD8] rounded-xl px-4 py-2 shadow-xs transition-colors cursor-pointer"
                    >
                ...
                filtered.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">Anchor Partner</p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Partner Bank */}
                    <td className="px-6 py-4.5">
                      <span className="inline-flex items-center gap-1.5 text-slate-700 text-[11px]">
                        <Building size={13} className="text-[#1F4DA8] shrink-0" />
                    ...
                    <td className="px-6 py-4.5 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          title={m.status === 'ACTIVE' ? 'Deactivate Manufacturer' : 'Activate Manufacturer'}
                        >
                          {togglingId === m.id ? (
