...
export const platformService = {
  ...
  async createBank(input: { bankCode: string; branchCode?: string; name?: string; bankName?: string; location?: string }): Promise<BankResponse> {
    if (USE_MOCKS) return platformMock.createBank(input as any);
    const payload: OnboardBankRequest = {
      bankCode: input.bankCode,
      branchCode: input.branchCode || '',
      name: input.name || input.bankName || 'Partner Bank',
      location: input.location,
    };
    const { data } = await apiClient.post<any>('/api/onboarding/banks', payload);
  ...
  async deleteBankAdmin(id: string): Promise<void> {
    ...
    try {
      await apiClient.delete(`/api/onboarding/banks/admins/${id}`);
    } catch {
      try {
        await apiClient.patch(`/api/onboarding/banks/admins/${id}/deactivate`);
      } catch {
