  ...
  const perms = (role.permissions || []).map((code) => {
    ...
    return (
      found || {
        ...
        module: 'General',
        stakeholderType: (role.tenantType as StakeholderType) || 'PLATFORM',
        description: code,
      }
    );
  });
    ...
    try {
      ...
      if (dtos && dtos.length > 0) {
        return dtos.map((dto) => ({
          id: dto.code,
          code: dto.code,
          name: dto.name || dto.description || dto.code.replace(/_/g, ' '),
          module: dto.category || 'Permissions',
          stakeholderType: (dto.tenantType as StakeholderType) || stakeholder || 'PLATFORM',
